...
export function PurchaseOrderFormPage() {
  ...
  const addItem = (productId: string) => {
    ...
    setItems((cur) => [
      ...cur,
      {
        ...
        productName: p.name,
        sku: p.sku,
        qty: Math.max(10, p.reorderLevel - p.stock),
        unitPrice: p.cost,
      },
    ]);
  };
  ...
  const canSubmit =
    supplierId && items.length > 0 && items.every((i) => i.qty > 0 && i.unitPrice >= 0);
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <PageHeader
        ...
        description="Select a supplier, add line items, and submit for fulfillment."
      />
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            {supplierId && (
              <select
                ...
                className="po-input !w-auto !h-9 text-xs"
              >
          ...
          {items.length === 0 ? (
            <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
              Select a supplier, then add products.
            </div>
          ) : (
            <div className="rounded-xl border border-border overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-muted/20 text-[11px] uppercase text-muted-foreground">
                    ...
                    <tr key={it.productId} className="border-t border-border">
                      ...
                      <td className="p-2">
                        <input
                          ...
                          className="po-input !h-9 text-right tabular-nums"
                        />
                      </td>
                      <td className="p-2">
                        <input
                ...
                <tfoot>
                  <tr className="bg-muted/20 border-t border-border">
                    <td
                      colSpan={3}
                      className="p-3 text-right text-xs uppercase tracking-wider text-muted-foreground font-semibold"
                    >
                      Total
                    </td>
                    <td className="p-3 text-right text-lg font-display font-bold text-gradient-gold tabular-nums">
                      ${total.toFixed(2)}
                    </td>
                    <td></td>
                  </tr>
