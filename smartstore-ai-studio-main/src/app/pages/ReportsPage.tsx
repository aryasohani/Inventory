...
export function ReportsPage() {
  ...
  const byCategory = Object.entries(
    productList.reduce<Record<string, number>>((acc, p) => {
  ...
  const weeklySpend = (orders.data ?? [])
    .slice(0, 6)
    .reverse()
    .map((o, i) => ({ week: `W${i + 1}`, spend: Math.round(o.total) }));

  return (
    <div className="space-y-6">
      <PageHeader
        title="Reports & Insights"
        description="Weekly summaries, category breakdowns, and AI-generated business insights."
      />
      ...
      <div className="relative overflow-hidden rounded-2xl border border-primary/25 p-5 bg-gradient-to-br from-primary/10 via-primary/4 to-transparent">
        <div className="flex items-start gap-3">
          <div className="size-10 rounded-xl bg-gradient-gold grid place-items-center shrink-0 shadow-glow">
          ...
          <div>
            ...
            <h3 className="font-display font-semibold text-lg mt-1">
              Operations are healthy with two areas of attention
            </h3>
            <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
              Demand grew <strong className="text-foreground">+8.2%</strong> week-over-week, driven
              by Beverages and Pharmacy. Inventory turnover improved to{" "}
              <strong className="text-foreground">5.4×</strong>. Two SKUs are at{" "}
              <strong className="text-destructive">critical stock</strong> — review the dashboard.
              Consider rebalancing <strong className="text-foreground">$3.2k</strong> from
              slow-moving Electronics into accelerating Beverages.
            </p>
          </div>
        </div>
      </div>
