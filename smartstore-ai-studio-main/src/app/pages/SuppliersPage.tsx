      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {(data ?? []).map((s) => (
            <div
              key={s.id}
              className="rounded-2xl border border-border bg-card p-5 shadow-card hover:border-primary/30 transition-all group"
            >
              ...
              <div className="mt-3 space-y-1.5 text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  <Clock className="size-3.5" /> {s.leadTimeDays}d lead time
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-border flex gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (confirm("Remove supplier?")) del.mutate(s.id);
                  }}
                >
