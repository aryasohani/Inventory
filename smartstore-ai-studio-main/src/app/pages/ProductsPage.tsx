            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/20 text-[11px] uppercase tracking-wider text-muted-foreground">
                  <Th>
                    Product <ArrowUpDown className="size-3 inline ml-1 opacity-40" />
                  </Th>
                  <Th>SKU</Th>
                  <Th>Category</Th>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((p, idx) => (
            ...
            >
              <Link
                to={`/products/${p.id}`}
                className="block rounded-2xl border border-border bg-card hover:border-primary/40 transition-all group p-4 shadow-card"
              >
