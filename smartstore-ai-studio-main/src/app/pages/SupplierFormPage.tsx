...
export function SupplierFormPage() {
  ...
  const form = useForm<V>({
    ...
    defaultValues: { name: "", email: "", phone: "", categories: "", leadTimeDays: 5 },
  });
  ...
  useEffect(() => {
    if (existing.data)
      form.reset({
        ...
        leadTimeDays: existing.data.leadTimeDays,
      });
  }, [existing.data, form]);
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-5">
          <Field label="Email" error={form.formState.errors.email?.message}>
            <input type="email" {...form.register("email")} className="form-input" />
          </Field>
          <Field label="Phone" error={form.formState.errors.phone?.message}>
            <input {...form.register("phone")} className="form-input" />
          </Field>
        </div>
        <Field
        ...
        <Field label="Lead time (days)" error={form.formState.errors.leadTimeDays?.message}>
          <input type="number" {...form.register("leadTimeDays")} className="form-input" />
        </Field>
