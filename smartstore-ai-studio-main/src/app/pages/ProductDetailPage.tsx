...
export function ProductDetailPage() {
  ...
  const forecast = useQuery({
    queryKey: ["products", id, "forecast"],
    queryFn: () => productsApi.forecast(id),
  });
  const history = useQuery({
    queryKey: ["products", id, "history"],
    queryFn: () => productsApi.history(id),
  });
  ...
  return (
    <div className="space-y-6">
      <Button asChild variant="ghost" size="sm" className="text-muted-foreground -ml-2">
        <Link to="/products">
          <ArrowLeft className="size-4 mr-1.5" /> Back to products
        </Link>
      </Button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <MiniStat
          ...
          label="Current Stock"
          value={p.stock}
          hint={`reorder at ${p.reorderLevel}`}
        />
        <MiniStat
          icon={<DollarSign className="size-4" />}
          label="Unit Price"
          value={`$${p.price.toFixed(2)}`}
          hint={`cost $${p.cost.toFixed(2)}`}
        />
        <MiniStat
          icon={<TrendingUp className="size-4" />}
          label="Velocity"
          value={`${p.velocity}/d`}
          hint={`${daysOfCover}d of cover`}
        />
        <MiniStat
          icon={<Calendar className="size-4" />}
          label="Days to Expiry"
          value={daysToExpiry}
          hint={format(new Date(p.expiryDate), "MMM d, yyyy")}
          accent={daysToExpiry < 14 ? "warn" : undefined}
        />
      </div>
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <div className="text-sm font-display font-semibold">
                Decision intelligence for this SKU
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Insight
              ...
              detail={
                daysToExpiry < 30
                  ? `Will expire before fully sold at current velocity`
                  : "Safe horizon"
              }
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={history.data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Line
                  ...
                  strokeWidth={2.5}
                  dot={{ fill: "oklch(0.70 0.13 230)", r: 3 }}
                  activeDot={{ r: 5 }}
                />
              </LineChart>
            </ResponsiveContainer>
          ...
          <Meta
            ...
            value={
              <span className="inline-flex items-center gap-1.5">
                <Truck className="size-3.5 text-muted-foreground" /> {p.supplierId}
              </span>
            }
          />
        </div>
...
function MiniStat({
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4">
      ...
      <div
        className={`text-2xl font-display font-bold tabular-nums ${accent === "warn" ? "text-warning" : ""}`}
      >
        {value}
      </div>
