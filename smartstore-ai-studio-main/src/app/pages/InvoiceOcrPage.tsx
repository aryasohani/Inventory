...
export function InvoiceOcrPage() {
  ...
  const parse = useMutation({
    ...
    onSuccess: (inv) => {
      setParsed(inv);
      toast.success("Invoice parsed", { description: `${inv.items.length} line items extracted` });
    },
  });
  const confirm = useMutation({
    mutationFn: () => invoicesApi.confirm(parsed!),
    onSuccess: () => {
      toast.success("Stock updated from invoice");
      setFile(null);
      setParsed(null);
    },
  });
  ...
  const updateItem = (idx: number, patch: Partial<Invoice["items"][number]>) => {
    ...
    setParsed({
      ...
      items: parsed.items.map((it, i) =>
        i === idx
          ? { ...it, ...patch, total: (patch.qty ?? it.qty) * (patch.price ?? it.price) }
          : it,
      ),
    });
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <PageHeader
        title="Invoice OCR"
        description="Upload supplier invoices — AI extracts line items and updates stock automatically."
      />

      {!parsed ? (
        <div className="rounded-2xl border-2 border-dashed border-border bg-card p-10 text-center">
          <div className="size-14 rounded-2xl bg-gradient-gold mx-auto grid place-items-center shadow-glow mb-4">
          ...
          <label className="mt-5 inline-flex">
            ...
            />
            <span className="inline-flex items-center px-4 h-10 rounded-xl border border-border bg-muted/30 hover:bg-muted/50 cursor-pointer text-sm font-medium transition-colors">
          ...
          {!file && (
            <Button
              onClick={() => {
                setFile(new File([], "demo-invoice.pdf"));
                setTimeout(() => parse.mutate(), 100);
              }}
              variant="outline"
              className="mt-4"
            >
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border border-success/30 bg-success/5 p-4 flex items-center gap-3">
            ...
            <div className="text-sm">
              <strong>Parsed successfully</strong> · review and edit fields below before confirming.
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5 pb-5 border-b border-border">
              <Field label="Supplier">
                <input
                  ...
                  onChange={(e) => setParsed({ ...parsed, supplierName: e.target.value })}
                  className="ocr-input"
                />
              </Field>
              <Field label="Invoice #">
                <input
                  ...
                  onChange={(e) => setParsed({ ...parsed, invoiceNumber: e.target.value })}
                  className="ocr-input"
                />
              </Field>
              <Field label="Invoice date">
                <input
                  type="date"
                  value={parsed.invoiceDate.slice(0, 10)}
                  onChange={(e) =>
                    setParsed({ ...parsed, invoiceDate: new Date(e.target.value).toISOString() })
                  }
                  className="ocr-input"
                />
              </Field>
            ...
            <table className="w-full text-sm">
                  ...
                  <tr key={idx} className="border-t border-border">
                    ...
                    <td className="py-2 px-1">
                      <input
                        ...
                        className="ocr-input text-right tabular-nums"
                      />
                    </td>
                    <td className="py-2 px-1">
                      <input
              ...
              <tfoot>
                <tr className="border-t border-border">
                  <td
                    colSpan={3}
                    className="py-3 text-right text-xs uppercase tracking-wider text-muted-foreground font-semibold"
                  >
                    Invoice total
                  </td>
                  <td className="py-3 text-right text-xl font-display font-bold text-gradient-gold tabular-nums">
                    ${total.toFixed(2)}
                  </td>
                </tr>
              </tfoot>
